      ...
      <unicloud-db ref="udb" :collection="collectionList" :where='where'   page-data="replace"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            ...
            <uni-th align="center" filter-type="range" @filter-change="filterChange($event, 'sort')" sortable @sort-change="sortChange($event, 'sort')">分类排序</uni-th>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        collectionList: [
					  db.collection('classifyType').getTemp(),
			          db.collection('uni-cms-categories').field('name as text, _id').getTemp(),
		],
        query: '',
        where: '',
        orderby: dbOrderBy,
