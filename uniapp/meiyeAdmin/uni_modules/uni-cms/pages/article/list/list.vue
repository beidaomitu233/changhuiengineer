...
<!--        </view>-->
		...
		<view class="uni-container">
			<unicloud-db ref="udb" :collection="collectionList" :where="where" page-data="replace" :orderby="orderby"
                ...
                @filter-change="filterChange($event, '_id')"
            >分类</uni-th>
              ...
              <view class="multi-tip" v-if="item.thumbnail && item.thumbnail.length >= 3">3图</view>
						...
						<uni-td align="center">
							<view class="uni-group">
								...
								<button v-if="hasPermission('DELETE_UNI_CMS_ARTICLE')" @click="confirmDelete(item._id,item.classifyType)" class="uni-button" size="mini"
									type="warn">删除</button>
							</view>
						</uni-td>
					</uni-tr>
  ...
  mixins: [authMixin],
	data() {
		return {
			...
			collectionList: [
				db.collection(articleDBName).field('_id,user_id,category_id,content,title,excerpt,article_status,view_count,like_count,is_sticky,is_essence,thumbnail,publish_date,publish_ip,last_modify_date,last_modify_ip,classifyType').getTemp(),
		...
		confirmDelete(id,classTypeID) {
			...
			this.$refs.udb.remove(id, {
				success: (res) => {
					 ...
					 */
						 utilobj.operation('classifyType','article_count',`${classTypeID}`,-1)
				}
			})
		},
		// 排序改变
