      ...
      <unicloud-db ref="udb" :collection="collectionList" :where="where" page-data="replace"
        ...
        <uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
            ...
            <uni-th align="center" sortable @sort-change="sortChange($event, 'picurl')">产品图</uni-th>
            ...
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'grade')" sortable @sort-change="sortChange($event, 'grade')">等级</uni-th>
            ...
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'pronum')" sortable @sort-change="sortChange($event, 'pronum')">型号</uni-th>
            ...
            <uni-th align="center" filter-type="search" @filter-change="filterChange($event, 'year')" sortable @sort-change="sortChange($event, 'year')">年份</uni-th>
          ...
          <uni-tr v-for="(item,index) in data" :key="index">
            ...
            <uni-td align="center">{{item.navid[0].text}}</uni-td>
            <uni-td align="center">
            ...
            <uni-td align="center">{{item.pronum}}</uni-td>
            <uni-td align="center">{{item.weight}}</uni-td>
            <uni-td align="center">{{item.year}}</uni-td>
            <uni-td align="center">{{item.checked == true ? '✅' : '❌'}}</uni-td>
            <uni-td align="center">
              <view class="uni-group">
...
<script>
  import { enumConverter, filterToWhere } from '../../js_sdk/validator/xzs_product_list.js';
  ...
  export default {
    data() {
      return {
        collectionList: [ db.collection('xzs_product_list').field('title,navid,picurl,orderid,grade,price,pronum,weight,year,checked').getTemp(),db.collection('xzs_product_nav').field('_id, classname as text').getTemp() ],
        query: '',
        where: '',
        orderby: dbOrderBy,
        orderByFieldName: "",
