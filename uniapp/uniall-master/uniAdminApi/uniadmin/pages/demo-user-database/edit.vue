<template>
  <view class="uni-container">
    <uni-forms ref="form" :model="formData" validateTrigger="bind">
      ...
      <uni-forms-item name="tel" label="手机">
        <uni-easyinput placeholder="手机号码" v-model="formData.tel" trim="both"></uni-easyinput>
      </uni-forms-item>
...
<script>
  import { validator } from '../../js_sdk/validator/demo-user-database.js';

  const db = uniCloud.database();
  const dbCmd = db.command;
  const dbCollectionName = 'demo-user-database';
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("username,tel,gender,avatar").get().then((res) => {
