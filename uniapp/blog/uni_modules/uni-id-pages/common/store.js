...
// 获取用户token过期时间
let tokenExpired=uniCloud.getCurrentUserInfo().tokenExpired - Date.now()
// 如果用户token过期就清除本地数据，要不然头像和名字会显示在本地
if(tokenExpired<0) hostUserInfo={}
console.log(tokenExpired);
const data = {
	userInfo: hostUserInfo,
	// 添加判断，tokenExpired减去当前时间，如果>0就是没过期
	hasLogin: Object.keys(hostUserInfo).length != 0 && tokenExpired>0
}
