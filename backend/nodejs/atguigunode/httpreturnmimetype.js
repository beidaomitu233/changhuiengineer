...
const server=http.createServer((request,response)=>{
  ...
  const url2=new URL(request.url,'http://localhost:9001')
   //获取请求文件类型，获取文件后缀，从 .开始截取，截取后存储在数组中，第【1】个元素就是文件类型。
   let suf=url2.pathname.split('.')[1]
   //拼接路径和文件名得到绝对路径
  let requestfilename=__dirname+url2.pathname
  requestfilename=path.resolve(requestfilename)

  let filetype={html:'text/html',
  css:'text/css',
  js:'text/javascript',
  png:'image/png',
  jpg:'image/jpeg',
  ...
  if(filetype[suf]){
        ...
        response.setHeader('Content-type',`${filetype[suf]}`)
  }else{
    //如果没有匹配导就设置为未知资源类型
